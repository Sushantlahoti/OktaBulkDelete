package com.okta.bulkdelete;

import static com.okta.bulkdelete.BulkDelete.*;
...
public class BulkDelete {
    ...
    public static void main(String args[]) throws Exception{
        ...
        System.out.println();
        System.out.println("Successfully deactivated "+successCount+" user(s)");
...
HttpUriRequest request = RequestBuilder
        .post("https://"+org+"/api/v1/users/"+csvRecord.get(csvLoginField)+"/lifecycle/deactivate")
        ...
        try{
            ...
            if (responseCode == 429){//Retry after appropriate time
                ...
                handleErrorResponse(false, responseCode, httpResponse, csvRecord, "");
                    ...
                    .delete("https://"+org+"/api/v1/users/"+csvRecord.get(csvLoginField))
                    .setHeader("Authorization", "SSWS " + apiToken)
                    .build();
                    CloseableHttpResponse deleteHttpResponse = null;
                    try{
                        deleteHttpResponse = httpclient.execute(deleteRequest);
                        int deleteResponseCode = deleteHttpResponse.getStatusLine().getStatusCode();
                       // System.out.println(csvRecord.get(csvLoginField) + "-" + deleteResponseCode);
                        if (deleteResponseCode == 429){//Retry after appropriate time
                            handleErrorResponse(true, deleteResponseCode, httpResponse, csvRecord, null);
                            long limitResetsAt = Long.parseLong(deleteHttpResponse.getFirstHeader("x-rate-limit-reset").getValue());
                            //Put this thread to sleep for at least 5 seconds
                            long timeToSleep = Math.abs(limitResetsAt - (System.currentTimeMillis()/1000)) + 5;
                            Thread.sleep(timeToSleep*1000);
                        }
                        else if (deleteResponseCode != 204 ){//Non-success
                            handleErrorResponse(false, deleteResponseCode, deleteHttpResponse, csvRecord, "");
                        }else   successCount.getAndIncrement();
